...
const Reservas: React.FC = () => {
  ...
  const handleCancelarReserva = async (id: number) => {
    ...
    try {
      await VentaService.cancelarVenta(id, { motivo: "Cancelado" });
  ...
  if (loading) return (
    <div className="h-96 flex flex-col items-center justify-center text-slate-400">
      ...
      <p className="font-bold text-[10px] uppercase tracking-[0.2em]">Cargando datos...</p>
    </div>
  );

  if (selectedReserva) {
    const paid = getMontoPagado(selectedReserva);
    const total = getTotal(selectedReserva);
    const vencida = estaVencida(selectedReserva.fechaVencimientoReserva);
    // Usamos pagosCredito según el DTO
    const historialPagos = selectedReserva.pagosCredito || [];
        ...
        <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            <div>
              ...
              <div className="flex items-center gap-4 mt-2">
                ...
                <span className={`text-sm font-bold flex items-center gap-1.5 ${vencida ? 'text-rose-600' : 'text-slate-500'}`}>
            ...
            <div className="text-right">
              ...
              <p className="text-4xl font-bold text-rose-600 tracking-tight leading-none">${(total - paid).toLocaleString()}</p>
            </div>
          </div>
            ...
            {activeTab === 'DETAILS' ? (
              <div className="space-y-4">
                {selectedReserva.detalles?.map((d: any, i: number) => {
                  // MAPEADO SEGÚN DetalleVentaResponse.java
                  const nombre = d.detalleProducto?.productoNombre || "Producto";
                  const codigo = d.detalleProducto?.codigo || "S/C";
                  const cantidad = d.cantidad || 0;
                  const precioUnit = d.precioUnitarioActual || 0;
                  const montoTotalLinea = d.precioTotalUnitario || 0;

                  return (
                    <div key={i} className="flex justify-between items-center p-5 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-indigo-500 border border-slate-100 shadow-sm">
                          <Package className="w-6 h-6" />
                        </div>
                        <div>
                          <p className="text-lg font-bold text-slate-800 leading-tight">{nombre}</p>
                          <div className="flex items-center gap-3 mt-1">
                             <span className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white px-2 py-0.5 rounded border">
                               <Tag className="w-3 h-3" /> Cod: {codigo}
                             </span>
                             ...
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                               P. Unit: ${precioUnit.toLocaleString()}
                             </span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Monto Línea</p>
                        <span className="font-bold text-2xl text-slate-900">${montoTotalLinea.toLocaleString()}</span>
                      </div>
                    </div>
                  );
                })}
                  ...
                  historialPagos.map((p: any, i: number) => (
                    <div key={i} className="flex justify-between items-center p-5 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-lg font-bold text-slate-800 leading-tight">Pago #{p.numeroPago || i+1}</p>
                          <p className="text-xs font-medium text-slate-400 mt-0.5">{formatFecha(p.fecha)}</p>
                        </div>
                      </div>
          ...
          <div className="p-10 bg-slate-50/80 border-t border-slate-100 flex justify-between items-center">
            <div className="flex gap-8">
              <div><p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Total Reserva</p><p className="text-xl font-bold text-slate-700">${total.toLocaleString()}</p></div>
              <div><p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Abonado</p><p className="text-xl font-bold text-emerald-600">${paid.toLocaleString()}</p></div>
            </div>
            <div className="flex gap-4">
              ...
              <button onClick={() => handleCancelarReserva(selectedReserva.id)} className="px-8 py-4 bg-white text-rose-500 rounded-[24px] font-bold text-sm uppercase border border-slate-200 hover:bg-rose-50 transition-all"><XCircle className="w-5 h-5 inline mr-2"/>Cancelar</button>
            </div>
          </div>
        </div>
        ...
        {showPaymentModal && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl p-10 space-y-8 animate-in duration-200">
              <h3 className="text-xl font-bold text-slate-900 uppercase">Registrar Abono</h3>
              <input type="number" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} placeholder="0.00" className="w-full text-4xl font-bold text-indigo-600 bg-slate-50 border border-slate-200 rounded-2xl p-8 focus:ring-4 focus:ring-indigo-500/10 outline-none" />
              <div className="flex gap-4">
                <button onClick={() => setShowPaymentModal(false)} className="flex-1 py-4 font-bold text-slate-400">Cerrar</button>
                <button onClick={handleConfirmPayment} className="flex-[2] py-4 bg-indigo-600 text-white rounded-[20px] font-bold hover:bg-indigo-700 shadow-xl shadow-indigo-100">Confirmar Pago</button>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div className="space-y-8 animate-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {reservas.length === 0 ? (
          <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-200"><p className="text-slate-400 font-bold uppercase tracking-widest">No hay reservas activas</p></div>
        ) : reservas.map(reserva => {
          const paid = getMontoPagado(reserva);
          const total = getTotal(reserva);
          const progress = reserva.progresoPago || 0; // Usamos progresoPago del DTO
          ...
          return (
            <div key={reserva.id} className="bg-white rounded-[40px] border border-slate-200 shadow-sm p-8 flex flex-col group hover:border-indigo-300 hover:shadow-xl transition-all">
              ...
              <div className="bg-slate-50 p-6 rounded-[24px] space-y-3 border border-slate-100">
                <div className="flex justify-between text-[9px] font-black uppercase"><p>{vencida ? 'Expirada' : 'Progreso de Pago'}</p><p>{Math.round(progress)}%</p></div>
                ...
                <div className="flex justify-between pt-2">
                  <div className="text-left"><p className="text-[9px] font-bold text-slate-400 uppercase">Total</p><p className="text-lg font-medium text-slate-700">${total.toLocaleString()}</p></div>
                  <div className="text-right"><p className="text-[9px] font-bold text-slate-400 uppercase">Pendiente</p><p className="text-2xl font-black text-rose-600">${(total - paid).toLocaleString()}</p></div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 mt-10">
                ...
                <button onClick={() => handleCancelarReserva(reserva.id)} className="py-4 bg-rose-50 text-rose-600 rounded-[20px] text-[11px] font-bold uppercase border border-rose-100"><XCircle className="w-4 h-4 inline mr-1"/> Cancelar</button>
              </div>
            </div>
          );
        })}
